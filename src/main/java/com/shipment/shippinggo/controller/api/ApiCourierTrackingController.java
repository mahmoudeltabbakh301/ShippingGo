...
public class ApiCourierTrackingController {
    ...
    public ResponseEntity<?> getCourierLocations(@AuthenticationPrincipal User user) {
        ...
        // Only ADMIN, MANAGER, SUPER_ADMIN, DATA_ENTRY, and FOLLOW_UP can view courier locations
        if (user.getRole() != Role.ADMIN && user.getRole() != Role.MANAGER && user.getRole() != Role.SUPER_ADMIN && user.getRole() != Role.DATA_ENTRY && user.getRole() != Role.FOLLOW_UP) {
